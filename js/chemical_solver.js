...
function solve(matrix) {
	...
	if (i == matrix.rowCount() - 1)
		throw "Solução tudo-zero";  // Unique solution with all coefficients zero
...
function parseEquation(tok) {
	...
	while (true) {
		...
		if (next == "=") {
			...
			throw {message: "Sinal de mais ou igual esperado", start: tok.position()};
		} else if (next == "+") {
			tok.consume("+");
			lhs.push(parseTerm(tok));
		} else
			throw {message: "Sinal de mais esperado", start: tok.position()};
	}
...
function parseTerm(tok) {
	...
	if (next != null && next == "^") {
		...
		if (next == "+")
			...
			throw {message: "Sinal(+ -) experado", start: tok.position()};
	...
	if (items.length == 0) {
		...
		if (items.length > 1)
			throw {message: "Termo invalido - elétron precisa ficar sozinho", start: startPosition, end: tok.position()};
		else if (charge != 0 && charge != -1)
			throw {message: "Termo invalido - carga invalida para o eletron", start: startPosition, end: tok.position()};
		// Tweak data
		items = [];
		charge = -1;
...
function Tokenizer(str) {
	...
	this.peek = function() {
		...
		if (match == null)
			throw {message: "Simbolo invalido", start: i};
		return match[0];
	};
